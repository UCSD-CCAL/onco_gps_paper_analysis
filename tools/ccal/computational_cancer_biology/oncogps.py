...
def define_components(matrix,
    ...
    plot_heatmap(
        matrix,
        title='Matrix to be Decomposed (rank-normalized columns)',
        xlabel='Sample',
        ylabel='Feature',
        xticklabels=False,
        yticklabels=False,
    ...
    nmf_results, cophenetic_correlation_coefficient = nmf_consensus_cluster(
        matrix,
        ks,
        n_jobs=n_jobs,
    ...
    plot_points(
        sorted(cophenetic_correlation_coefficient.keys()), [
...
def solve_for_components(w_matrix,
                         a_matrix,
                         method='nnls',
                         average_duplicated_rows_of_a_matrix=True,
...
def select_features_and_nmf(testing,
    ...
    features = feature_scores.index[:
                                    n_top_features] | feature_scores.index[-n_bottom_features:]
...
def define_states(matrix,
    ...
    matrix = normalize_2d_or_1d(
        normalize_2d_or_1d(matrix, '-0-', axis=1).clip(-max_std, max_std),
        method='0-1',
        axis=1)
...
def _compute_sample_coordinates(component_x_coordinates, component_x_samples,
    ...
    sample_coordinates = empty((component_x_samples.shape[1],
                                component_x_coordinates.shape[1]))
    ...
    for i, (_, c
        ...
        for d in range(component_x_coordinates.shape[
                1]):  # Compute coordinate in each dimension
...
def _plot_onco_gps(
    ...
    for i in components.index:
        ...
        if convexhull_region.contains_point(
            (components.ix[i, 'x'] + h_shift,
             components.ix[i, 'y'] + v_shift)):  # Flip
            h_shift *= -1
            v_shift *= -1
        x += h_shift
        y += v_shift
    ...
    if isinstance(annotation_grids, ndarray):
        ...
        for i in range(grids.shape[0]):
            for j in range(grids.shape[1]):
                ...
                if convexhull_region.contains_point((fraction_grids[i],
                                                     fraction_grids[j])):
                    if 0 < grids[i, j]:
                        c = (1, 0, 0)
                    else:
    ...
    if not samples.ix[:, 'annotation'].isnull().all():
        ...
        if annotation_type == 'continuous':
            ...
            samples.ix[:, 'annotation_for_plot'] = normalize_2d_or_1d(
                samples.ix[:, 'annotation'], '-0-').clip(-std_max, std_max)
        ...
        if samples.ix[:,
                      'annotation'].dtype == object:  # Plot categorical legends below the map
